...
namespace CoreLibrary.Utilities
{
    public class RequestHelper
    {
        ...
        {
            var header = HttpContextConfig.CurrentHttpContext.Request.Headers;
            const string key = "User-Agent";
            return header.ContainsKey(key) ? string.Empty : header[key].FirstOrDefault();
        }
        ...
        public static bool BrowserIsMobile()
        {
            ...
            if (!string.IsNullOrWhiteSpace(header[key]))
            {
                ...
                if (browsername.Length < 4)
                    return false;

                if (MobileCheck.IsMatch(browsername) || MobileVersionCheck.IsMatch(browsername.Substring(0, 4)))
                    return true;
            }
